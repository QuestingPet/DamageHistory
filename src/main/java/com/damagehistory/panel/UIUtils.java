...
public final class UIUtils {
    ...
    public static Color getDamageColor(int damage, DamageHistoryConfig config) {
        if (damage >= config.highDamageThreshold()) {
            return config.highDamageColor();
        }
        if (damage >= config.mediumDamageThreshold()) {
            return config.mediumDamageColor();
        }
    ...
    public static Color getTickDelayColor(int ticksSince, int attackSpeed, DamageHistoryConfig config) {
        if (ticksSince <= attackSpeed) {
            return config.goodTimingColor();
        }
        if (ticksSince <= attackSpeed + config.tickDelayTolerance()) {
            return config.okayTimingColor();
        }
